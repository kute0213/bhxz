...
// 全站背景图片响应式加载：按设备屏幕宽度（含 DPR）就近请求最合适的档位，
// 图片加载完成后再显示，避免移动端拉取 1920 主图造成浪费与闪烁
(function initResponsiveBackground() {
    var bg = document.querySelector('.site-background');
    if (!bg) return;
    var baseUrl = bg.getAttribute('data-background-url');
    if (!baseUrl) return;

    var SIZES = [768, 1280, 1920];

    function pickSize() {
        // 用物理像素宽度（CSS 宽度 × DPR）匹配服务端档位
        var width = Math.ceil((window.innerWidth || 1280) * (window.devicePixelRatio || 1));
