...
- **实时终端**：进入**独立实时终端页面**，支持实时流式输出（已移除旧版弹窗终端）
- **快捷命令**：以卡片网格展示，点击"运行"直接执行（Shell 命令会在终端页自动执行，脚本会在终端页加载并运行）
...
点击命令卡片底部的「运行」按钮，跳转到独立实时终端页并自动执行：

- **脚本命令类型（Shell）**：自动切到实时终端页并发送该命令，流式显示输出
- **脚本类型**：通过后端 SSE API 在独立子进程中执行，输出实时回流显示在终端页
...
> 旧版「弹窗终端」已移除，实时终端统一为**独立实时终端页面**（`/admin/script/terminal-page`）。在脚本控制台点击顶部「实时终端」按钮进入，关闭浏览器标签即结束会话。

#### 3.1 打开终端

点击页面顶部的「实时终端」按钮，进入独立实时终端页面，页面加载即自动连接持久 Shell 会话。

#### 3.2 执行命令

在底部输入框中输入命令，按 `Enter` 或点击执行按钮。命令通过 **SSE（Server-Sent Events）** 流式返回输出，实时逐行显示。
...
- **清屏**：点击顶部「清屏」按钮（或 `Ctrl+L`）清除终端内容
- **重置会话**：点击顶部「重置会话」按钮，丢弃当前 Shell 状态并重新建会话
