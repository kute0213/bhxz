...
基于 Flask 的 Minecraft 服务器社区门户，采用磨砂玻璃（Glassmorphism）设计风格。提供用户系统、社区投票与征集、模组介绍、管理后台、服务器性能监控、脚本控制台与 MiniScript 脚本引擎等功能。

## 文档索引

| 文档 | 说明 |
|------|------|
| 本文档 | 项目总览、快速开始、功能特性、配置、API、架构、脚本控制台使用说明 |
...
| POST | `/admin/script/run-script` | MiniScript 执行（SSE 流式+交互） |
...
本文档详细介绍 脚本控制台的快捷命令、实时终端、MiniScript 脚本语言、专业脚本编辑器的使用方法。
...
- **快捷命令**：以卡片网格展示，点击"运行"直接执行——Shell 命令/脚本都会打开**弹窗终端**（基于 xterm.js）并在其中自动执行，输出实时回流
- **脚本**：以卡片网格展示，点击"运行脚本"在**弹窗终端**中加载并执行
- **脚本编辑器**：进入专业脚本编辑器页面，提供语法高亮、代码补全、错误诊断等功能
- **定时任务**：进入定时任务管理页面

> **提示**：MiniScript 脚本通过"快捷命令"功能管理，也可在「脚本编辑器」中编写并保存。脚本统一在后端独立子进程中执行，通过 SSE 流式回流输出。

> **弹窗终端无独立入口**：不再有「实时终端」按钮。点击任意快捷命令/脚本的「运行」即自动打开弹窗终端并执行；独立的实时终端页面（`/admin/script/terminal-page`）仍可通过 URL 访问，供需要独立全屏终端的场景使用。
...
点击命令卡片底部的「运行」按钮，弹窗终端自动打开并在其中执行：

- **脚本命令类型（Shell）**：命令发送到共享持久 Shell 会话（PTY）执行，输入回车都会被真实驱动执行，输出通过 SSE 实时回流到弹窗终端
- **脚本类型**：通过后端 SSE API 在独立子进程中进行交互式执行，输出实时回流显示在弹窗终端（`alert`/`prompt`/`confirm` 会弹出对应的交互弹窗）

弹窗终端右上角提供「中断 / 清屏 / 重置 / 关闭」按钮：脚本运行时显示「中断」用于强制终止脚本；`Esc` 或点击遮罩可关闭弹窗。

#### 2.5 命令类型说明

| 类型 | 标签颜色 | 执行位置 | 说明 |
|------|----------|----------|------|
| 脚本命令 | 绿色 | 服务端 | 原始 Shell 命令，通过 SSE 流式返回输出 |
...
> 终端基于 **xterm.js**（业界标准终端模拟器）渲染，输入/输出/清屏/光标控制与本地终端完全一致，彻底替代旧版自制 ANSI 渲染器。
>
> **弹窗终端无独立入口**：点击快捷命令或脚本卡片的「运行」即打开弹窗终端并自动执行。此外，独立的实时终端页面（`/admin/script/terminal-page`）仍可通过 URL 直接访问，用于需要全屏独立终端的场景，页面加载即自动连接持久 Shell 会话。

#### 3.1 打开终端

- **弹窗终端**：在脚本控制台点击任意快捷命令/脚本的「运行」，弹窗终端自动打开并在其中执行对应内容；关闭弹窗不影响已建立的持久 Shell 会话
- **独立终端页**：直接访问 `/admin/script/terminal-page`，加载即自动连接同一持久 Shell 会话

#### 3.2 执行命令

终端使用真实伪终端（PTY）驱动 Shell 会话：直接在终端中键入命令，按 `Enter` 即可执行，输入回显、行编辑、`Tab` 补全、历史命令由终端驱动原生响应；输出通过 **SSE（Server-Sent Events）** 实时逐行回流。弹窗终端顶栏标题会显示当前正在执行的快捷命令。
...
- **清屏**：点击顶栏「清屏」按钮（或 `Ctrl+L`）清除终端内容
- **重置会话**：点击顶栏「重置」按钮，丢弃当前 Shell 状态并重新建会话
- **中断**：脚本运行时顶栏显示「中断」按钮，点击即强制终止正在运行的脚本
- **关闭**：点击顶栏关闭按钮或按 `Esc`（弹窗终端）关闭终端界面；独立终端页可直接关闭浏览器标签
...
脚本在独立子进程中运行，**不限制循环次数与运行时长**，可一直执行到结束；退出网页或点击「强制终止」时才会被回收（详见 [第 6 节：执行模型](#6-执行模型)）。
...
| `set_timeout(seconds)` | 秒数（int） | 无 | **已废弃**，脚本已无运行时长限制，此函数调用无副作用，仅为兼容旧脚本保留 |
...
MiniScript **不设任何安全限制**：不拦截危险函数、不限制语法、不做 AST 沙箱校验、不限循环次数、不限运行时长，脚本具备完整 Python 能力（含 `os`/`subprocess`/`import` 任意模块）。

**仅有以下防线**，确保脚本意外不会炸掉服务器：

> 1. **访问控制**：仅管理员可执行脚本（`/admin/script/*` 路由需 admin 权限）
> 2. **进程隔离**：脚本运行在独立子进程中，崩溃或异常不影响 Flask 主服务
> 3. **强制终止**：退出网页、关闭标签页或点击「强制终止」，脚本都会被回收

**运行时长**：脚本默认无超时，可一直运行。后台监控线程会在页面退出后强制终止失去连接的脚本，避免僵尸进程长期占用资源。
...
由用户在 脚本控制台或脚本编辑器点击「运行」触发，采用交互模式执行。
...
- **任务级超时**：每个定时任务可单独设置「最大超时时间（秒）」，超时自动终止；留空则使用全局默认（`TASK_EXECUTION_TIMEOUT`）
- **执行流程**：调度器每秒按到期时间排队判断 → 异步执行 → 启动子进程 → 输出与状态写入日志表
...
- **实时终端页面**：脚本执行时底部显示红色「中断」按钮，点击即中止脚本
...
| 场景 | 触发条件 | 行为 |
|------|----------|------|
| 页面退出 | 用户退出网页、关闭标签页/浏览器（含意外崩溃） | 前端上报 + 后台监控线程兜底，强制终止子进程 |
| 定时任务超时 | 定时任务超过其「最大超时时间」 | 终止子进程并记录日志 |
...
**已无运行时长限制。** 新版不再限制脚本的循环次数与运行时长，脚本可一直执行；退出网页或被用户点击「强制终止」时才结束（子进程随之回收，不会影响服务器）。

- 定时任务：可在创建时单独设置「最大超时时间（秒）」，超时自动终止
- 持续运行：在定时任务列表点「立即执行」直接运行，不受超时限制，可在「运行中任务」查看
...
**均不能再用作旧语义。** `set_interval` 与旧式 `set_timeout(代码字符串, 毫秒)` 早已移除。当前 `set_timeout(seconds)` 只是一个废弃的空函数，**不影响任何执行**（脚本已无运行时长限制）；旧脚本里若还有调用也不报错。

| 旧写法（已移除/不再生效） | 当前做法 |
|---------------------------|----------|
| `set_timeout("code", 3000)` — 3 秒后执行字符串 | 直接用 Python 原生流程控制 |
| `set_interval("code", 1000)` — 每秒重复 | `while True:` + `sleep(1)` |
| `clear_timer(id)` | （已移除） |
...
不可以。所有 `/admin/script/*` 路由均需要管理员权限，非管理员访问返回 403。

#### Q: 循环会无限运行吗？

可以。新版不再限制循环次数与运行时长，脚本可一路运行到底。终止方式：

1. **手动终止**：脚本运行时显示「强制终止」按钮，点击可立即停止（详见 [第 8 节](#8-强制终止)）
2. **退出网页**：关闭标签页/浏览器时脚本自动被强制终止
3. **进程隔离**：脚本运行在独立子进程中，即使异常崩溃也不影响 Flask 主服务

```
循环没有迭代次数上限，可无限运行（脚本无时间限制）
```

```
脚本默认无超时，退出网页或点击「强制终止」时结束
```
...
- 在 脚本控制台页面顶部点击紫色「脚本编辑器」按钮
- 或在快捷命令卡片上点击代码图标（紫色），可直接打开并编辑该命令

访问地址：`/admin/script/editor`（仅管理员）
...
> 新版已移除 AST 沙箱校验与函数黑名单，脚本不再因危险调用（如 `exec`/`eval`/`__import__`、双下划线属性）而被拦截。
...
定时任务功能允许管理员设置自动定时执行的 脚本命令或 MiniScript 脚本，所有任务在后台线程中异步执行，不会阻塞 Web 请求。

#### 13.1 访问方式

- 在 脚本控制台页面点击「定时任务」按钮
- 直接访问 `/admin/script/scheduled`
...
5. 选择调度类型
6. 根据调度类型设置间隔或执行时间
7. **设置最大超时时间（秒）**：单独为任务指定超时，超过即自动终止；留空则使用全局默认（`TASK_EXECUTION_TIMEOUT`）
8. 点击「保存」
...
- **删除**：删除任务（关联日志一并清理）
- **立即执行**：直接运行一次脚本，**不受超时限制**，可一路运行到底
- **运行中任务**：页面顶部实时面板展示当前正在执行的任务，方便跟踪「立即执行」启动的常驻脚本
- **查看日志**：查看该任务的执行历史和输出

> 调度器按**到期时间升序排队**，每 1 秒判断一次到期任务，不再全表扫描，效率更高。
...
- 页面每 15 秒自动通过 `/admin/script/scheduled/status` 接口拉取最新状态（轻量接口，仅返回状态信息）
...
| `cmd_run_logs` | `MAX_CMD_LOGS` | 1000 | 脚本命令执行日志 |
