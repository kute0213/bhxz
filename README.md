...
**留言板附件功能优化**
- 新增文件类型白名单限制：仅允许 `png/jpg/jpeg/gif/webp/pdf/txt/zip/rar/7z/doc/docx/xls/xlsx/ppt/pptx/mp4/mp3/wav`
- 新增文件数量限制：单次回复最多上传 **5 个**附件
- 新增文件大小限制：单个文件最大 **100MB**
- 附件存储格式升级：从纯文件名数组升级为 JSON 元信息数组，包含 `filename`、`original_name`、`file_type`、`size_bytes`
- 前端附件预览：上传时实时显示图片缩略图/文件图标、文件名、文件大小
- 前端实时校验：选择文件后立即检查数量、类型、大小，超出限制时给出明确错误提示
- 附件展示优化：历史附件按类型显示对应图标（图片、PDF、压缩包、音视频等），点击可下载
