...
│   ├── logger.py                 #   控制台日志工具（统一格式输出，自动使用 get_client_ip 获取真实 IP）
...
**性能监控页面 500 错误修复：**
- 修复 `/performance` 页面渲染时 `url_for('api.performance')` 报错：端点名应为 `api.api_performance`（函数名），而非 `api.performance`（路由路径）

**广播邮件页面修复与优化：**
- 修复发送按钮无反应：确认弹窗位于 `page_modals` 块，原脚本在 `content` 块中执行时弹窗元素尚未加载，已将脚本移至 `extra_script` 块确保元素已就绪
- 修复 `var body` 与 `document.body` 的变量名冲突，提升代码健壮性
- 实时预览升级为分栏 Markdown 编辑器：左侧编辑区 + 右侧实时预览区，支持工具栏（加粗/斜体/标题/列表/代码/链接/引用）、滚动同步，与指南编辑器和讨论帖编辑器保持一致
