...
def api_bind():
    ...
    # 基础校验
    if not server_path:
        return jsonify({'success': False, 'message': '游戏根目录不能为空'}), 400
    if not username:
    ...
    try:
        ...
        conn.execute(
            ...
            (user['id'], result['username'], now),
        )
        conn.commit()

        result['success'] = True
        result['message'] = f"账号 '{result['username']}' 绑定成功"
