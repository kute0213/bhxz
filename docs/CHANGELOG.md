...
* **一键更新统一使用 ZIP 下载（跳过 git）**：无论本地是否为 git 仓库，`update.py` 始终走 ZIP 下载覆盖方式，避免 git 拉取受镜像限速、合并冲突等问题；所有可用镜像依次尝试，全部失败才报错
...
* **数据库 disk I/O error（WAL 模式根因修复）**：`core/db/connection.py` 的 `_create_connection()` 在执行 `PRAGMA journal_mode=WAL` 时，若残留的 `-wal`/`-shm` 文件损坏或文件系统不支持 WAL（FAT32/网络盘），SQLite 抛出 `disk I/O error` 导致应用启动失败。修复：WAL 失败后自动清理残留 `-wal`/`-shm` 文件并重试，仍然失败则降级到 `TRUNCATE` 日志模式保证可用性；同时在连接前确保 `db` 目录存在（双保险，即使 `config.py` 的 makedirs 被跳过也能恢复）
