...
网站将用户头像和管理员维护的全站首页背景图库存入私有
MinIO Bucket。普通用户只能管理自己的头像；全站背景只能由管理员在
「管理中心 → 系统设置」上传、选择或恢复默认。

用户头像使用 `users/{user_id}/avatar.webp`，背景图库使用
`site/backgrounds/{uuid}.webp`。上传新背景不会覆盖旧图，可在管理后台随时切换。
