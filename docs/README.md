...
- **快捷命令**：点击卡片「运行」即打开弹窗终端（基于 xterm.js）并自动执行，输出实时回流
- **脚本**：以卡片网格展示，点击「运行脚本」在弹窗终端中加载并执行
- **脚本编辑器**：进入专业脚本编辑器页面，提供语法高亮、代码补全、错误诊断等功能
- **定时任务**：进入定时任务管理页面

> **提示**：MiniScript 脚本通过"快捷命令"功能管理，也可在「脚本编辑器」中编写并保存。脚本统一在后端独立子进程中执行，通过 SSE 流式回流输出。

> **弹窗终端无独立入口**：不再有「实时终端」按钮，点击任意快捷命令/脚本的「运行」即自动打开弹窗终端并执行；独立实时终端页面（`/admin/script/terminal-page`）仍可通过 URL 访问。
...
> 终端基于 **xterm.js**（业界标准终端模拟器）渲染，输入/输出/清屏/光标控制与本地终端完全一致。弹窗终端无独立入口，点击快捷命令或脚本卡片的「运行」即打开；独立的实时终端页面（`/admin/script/terminal-page`）仍可通过 URL 直接访问，加载即自动连接持久 Shell 会话。

#### 3.1 打开终端

- **弹窗终端**：在脚本控制台点击任意快捷命令/脚本的「运行」，弹窗终端自动打开并执行对应内容
- **独立终端页**：直接访问 `/admin/script/terminal-page`，加载即自动连接同一持久 Shell 会话

#### 3.2 执行命令

终端使用真实伪终端（PTY）驱动 Shell 会话：直接键入命令，按 `Enter` 即可执行，输入回显、行编辑、`Tab` 补全、历史命令由终端驱动原生响应；输出通过 **SSE（Server-Sent Events）** 实时逐行回流。
