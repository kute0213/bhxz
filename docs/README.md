...
MiniScript 是一种 **Python 子集脚本语言**，由后端执行引擎在独立 Python 子进程中执行。前端编辑器（Monaco）仅负责代码编辑与高亮，不再解释执行。

支持完整 Python 语法：控制流（if/elif/else、while、for）、函数（def）、类（class）、异常处理（try/except）、`import` 标准库、推导式、f-string、装饰器等。执行前通过 AST 沙箱校验，禁止危险调用（详见 [第 6 节：安全限制](#6-安全限制)）。
...
> MiniScript 使用 Python 原生类型：`True`/`False`/`None`（注意首字母大写），不再使用旧的 `true`/`false`/`null`。
...
> **注意**：MiniScript 不再提供 `append(list, ...)` / `push(list, item)` / `pop(list, idx?)` / `slice(list, ...)` 等自定义函数，请直接使用 Python 列表方法 `list.append(x)`、`list.pop()`、切片 `list[a:b]` 等。
...
支持默认参数、关键字参数、`*args` / `**kwargs`、返回多个值（元组解包）：
...
允许 `import` Python 标准库（`import` 语句在 AST 白名单中）：
...
> **注意**：AST 沙箱仅拦截对 `exec`/`eval`/`__import__()` 等函数的**直接调用**，不限制 `import` 语句导入的模块。由于 MiniScript 仅管理员可执行，且运行在独立子进程中（不影响 Flask 主服务），因此信任管理员使用 `import` 的判断。详见 [第 6 节：安全限制](#6-安全限制)。
