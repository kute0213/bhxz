...
# 自动封禁时长（分钟），0 表示永久封禁；可在管理后台 → 系统设置中热更新
AUTO_BAN_DURATION_MINUTES = int(os.environ.get('AUTO_BAN_DURATION_MINUTES', '30'))

# 各可疑操作自动封禁子开关（与 SETTINGS_REGISTRY 中同名设置对应）
...
QQ_GROUP_URL = (
    ...
    '&authKey=rMtk0BTqbTh2Dx%2BwtNX3GwhYs4NEZDPuKTO0UBHc6X2r55iPkda3lmuA9Styubor'
    '&busi_data=eyJncm91cENvZGUiOiI1OTY3OTQxMTIiLCJ0b2tlbiI6IkNUajRvZ0NJQ1dzQ3lZb0FBSzdRSElFQXdqOWZaZy9QSFRlajhuMGpIeUU5bkNyQnA1WGloQW0zcFVReXJpakoiLCJ1aW4iOiIzODQ2NDE1NDczIn0%3D'
    '&data=U8tnRnk8Yo1OQFedRkDUVccnyfIRXgZja1nQxv60UTRAphySRL7G3XPXtqrubu0Th2TJ4Q_l-BgqRjikCRI5_Q'
...
# Sitemap
    ...
    # MC 游戏目录
    ('MC_GAME_FOLDER', '', 'str', 'MC 游戏文件夹', 'Minecraft 服务器游戏目录的绝对路径（如 /home/mc/server），用于自动发现 EasyAuth 数据库', 'RCON 配置'),
    ...
    # 审核限制
    ('MAX_PENDING_CONTENT', 5, 'int', '用户最多待审核内容数', '用户同时处于待审核状态的内容（建筑/指南/音频/背景）数量上限，超过后无法继续发布需要审核的内容。管理员不受此限制。', '内容审核'),
